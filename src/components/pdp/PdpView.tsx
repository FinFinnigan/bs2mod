...
export function PdpView({ pdp }: { pdp: ProductPDP }) {
  ...
  function addToBag() {
    ...
    show(`Added to bag · ${[selectedVariant.size, selectedVariant.colour].filter(Boolean).join(" ")}`);
  }

  const soldOut = selectedVariant != null && selectedVariant.stock <= 0;

  return (
    <div className="container pdp-page" style={{ paddingTop: "var(--space-4)" }}>
      <Breadcrumb
        items={[
          { label: "Home", href: "/" },
      ...
      <div style={{ display: "grid", gap: "var(--space-8)" }} className="pdp-layout">
        <Gallery images={pdp.gallery} />

        <div className="pdp-info" style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 6 }}>
              {pdp.reviewSummary.count > 0 ? (
                <a href="#reviews" style={{ display: "inline-flex", alignItems: "center", gap: 8, color: "var(--color-ink)", padding: "12px 0", margin: "-12px 0" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ display: "flex", gap: "var(--space-4)", flexWrap: "wrap", fontSize: "var(--fs-caption)", color: "var(--color-ink-muted)" }}>
              ...
              <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                <IconTruck size={16} /> Free delivery over €50
              </span>
              <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
      ...
      <section id="reviews" style={{ marginTop: "var(--space-12)", maxWidth: 720 }}>
        ...
        {pdp.reviewSummary.count > 0 ? (
          ...
          <p style={{ color: "var(--color-ink-muted)" }}>No reviews yet — be the first to review this product.</p>
        )}
