...
export function ActiveFilterChips({
  ...
  if (active.minPrice != null && active.maxPrice != null) {
    entries.push({ key: "price", value: "", label: `€${active.minPrice}–${active.maxPrice}` });
  } else {
    if (active.minPrice != null)
      entries.push({ key: "minPrice", value: "", label: `From €${active.minPrice}` });
    if (active.maxPrice != null)
      entries.push({ key: "maxPrice", value: "", label: `Under €${active.maxPrice}` });
  }
      ...
      {entries.map((e) => (
        <span key={`${e.key}-${e.value}-${e.label}`} className="chip">
          ...
          <button
            ...
            style={{ border: "none", background: "transparent", fontWeight: 700, fontSize: 14, lineHeight: 1, padding: "15px 18px", margin: "-15px -18px", color: "var(--color-ink)" }}
          >
            ×
          </button>
        </span>
      ))}
      <button type="button" onClick={onClearAll} style={{ border: "none", background: "transparent", color: "var(--color-primary)", fontSize: "var(--fs-caption)", fontWeight: 600, textDecoration: "underline", padding: "15px 0", margin: "-15px 0" }}>
        Clear all
      </button>
    </div>
  );
