...
// Order confirmation view ÔÇö reads the opaque orderToken from the URL and asks the
...
// payment session exists but has NOT settled ÔÇö async methods such as bank
// transfer legitimately sit here ÔÇö so they get "pending" copy, never "confirmed".
  ...
  switch (status) {
    ...
    case "placed":
      return {
        ...
        body: "Your order is placed and reserved, but your payment hasn't settled yet. We'll confirm it and email you as soon as it does ÔÇö there's no need to order again.",
...
function variantText(item: OrderItemView): string {
  return [item.size, item.colour].filter(Boolean).join(" ┬À ");
}
  ...
  if (state.kind === "missing-token") {
    return (
      <Notice
        ...
        body="This page needs an order reference, and none was provided. If you've just placed an order, open the link from your confirmation email ÔÇö or head back to checkout."
      >
  ...
  if (state.kind === "error") {
    return (
      <Notice
        ...
        body="Something went wrong while loading this order's latest status. Your order is safe ÔÇö please try again in a moment."
      >
...
export function ConfirmationLoading() {
  return (
    ...
    >
      ...
      <span className="visually-hidden">Loading your order detailsÔÇª</span>
    </div>
  );
}
