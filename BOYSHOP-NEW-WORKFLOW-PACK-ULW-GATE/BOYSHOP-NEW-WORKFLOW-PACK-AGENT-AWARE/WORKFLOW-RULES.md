...
- ULW is declared in chunk metadata, not decided dynamically at launch.
- If the active chunk contains `ULW: REQUIRED`, stop before implementation/routing discovery and wait for owner action.
- Gate message: `⛔ ULW REQUIRED FOR <CHUNK-ID> — execution paused before implementation. Switch model and/or enable ULW, then reply CONTINUE <CHUNK-ID>.`
- `CONTINUE <CHUNK-ID>` satisfies the gate for that chunk in the current session; do not stop twice for the same chunk.
- Chunks without `ULW: REQUIRED` execute normally; do not run an ULW-needs assessment.
