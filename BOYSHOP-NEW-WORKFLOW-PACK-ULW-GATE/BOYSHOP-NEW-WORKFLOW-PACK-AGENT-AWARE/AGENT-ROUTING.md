...
If the best agent would require an unauthorized provider/model escalation, report that separately and follow `MODEL-ROUTING.md`. Do not convert that situation into `ULW: REQUIRED` at runtime; ULW must already be explicit in the active chunk metadata.
...
Every generated future chunk must also carry explicit execution-mode metadata:

- `ULW: REQUIRED` when the pack author/dynamic-maintenance step has already classified it as an ULW chunk; otherwise
- `ULW: NOT REQUIRED`.

Do not defer this decision to chunk launch.
