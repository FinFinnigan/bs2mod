# Create Mollie verification document
...
## Outcome
Create the Mollie verification document from scratch using only C00.4 evidence. C00.4 confirmed no Mollie verification document exists in either tree; legacy `C:\dev\BoyShop` copies are historical reference only. Record the verification evidence documented in C00.4 (`src/lib/backend/payments/adapters/mollie.ts`, `src/lib/backend/payments/__tests__/mollie.test.ts`, `scripts/verify-e2e-payment.cjs`). Do not fabricate live-mode facts; do not include secrets.
