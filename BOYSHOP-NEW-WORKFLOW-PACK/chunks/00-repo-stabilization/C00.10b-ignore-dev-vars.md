# Ignore Cloudflare local secrets (.dev.vars)
...
## Outcome
Tiny edit. Guard Cloudflare's standard local-secrets file: add `.dev.vars` (and `.dev.vars.*`, keeping `.dev.vars.example` trackable) to `BS2Mod-site/.gitignore`. Verify with `git check-ignore` and a temporary-file probe, then remove the probe. No other project-tree changes.

Why: C00.10 audit found `.dev.vars` is matched by no ignore rule — a temporary probe showed `?? BS2Mod-site/.dev.vars` in `git status`, so it would be swept up by a future `git add`. `wrangler dev`/`wrangler deploy` consume `.dev.vars` for local credentials (DB URL, Mollie keys, API tokens); leaving it unguarded is a secret-commit risk.
